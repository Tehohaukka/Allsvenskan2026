...
def _render_match(match: dict) -> None:
    ...
    with st.expander(f"**{match['home']} vs {match['away']}**{result_str}", expanded=True):
        ...
        cols[1].metric("Close 1", _pct(c_p1), delta=_delta(m["p1"], c_p1))
        cols[2].metric("Malli X", _pct(m["pX"]))
        cols[3].metric("Close X", _pct(c_pX), delta=_delta(m["pX"], c_pX))
        cols[4].metric("Malli 2", _pct(m["p2"]))
        cols[5].metric("Close 2", _pct(c_p2), delta=_delta(m["p2"], c_p2))
        ...
        cols[1].metric("Close koti", _pct(c_ah_home), delta=_delta(ah_m["home"], c_ah_home))
        cols[2].metric("Malli vieras", _pct(ah_m["away"]))
        cols[3].metric("Close vieras", _pct(c_ah_away), delta=_delta(ah_m["away"], c_ah_away))
        ...
        cols = st.columns(4)
        cols[0].metric("Malli over", _pct(tot_m["over"]))
        cols[1].metric("Close over", _pct(c_over), delta=_delta(tot_m["over"], c_over))
        cols[2].metric("Malli under", _pct(tot_m["under"]))
        cols[3].metric("Close under", _pct(c_under), delta=_delta(tot_m["under"], c_under))
